...
/// @dev Minimal ParticipantRegistry interface used by getSummaryFiltered.
interface IParticipantRegistryExt {
...
contract ReputationRegistryUpgradeable is OwnableUpgradeable, UUPSUpgradeable {
    ...
    /**
     * @notice Reputation summary filtered to feedback from trusted institutions.
     *
     * Instead of accepting a caller-supplied client list, this function iterates
     * all clients that have ever given feedback for the agent and includes only
     * those whose wallet is a registered minter for one of the `trustedParticipantIds`.
     * This prevents an institution from gaming its own agents' scores by submitting
     * self-serving feedback from addresses it controls (P3 anti-gaming control).
     *
     * @param agentId               The agent to query.
     * @param trustedParticipantIds Allowlist of participant IDs whose feedback counts.
     * @param participantRegistry_  Address of a deployed ParticipantRegistry.
    ...
    function getSummaryFiltered(
        uint256        agentId,
        bytes32[]      calldata trustedParticipantIds,
        address        participantRegistry_,
        string calldata tag1,
        string calldata tag2
    ) external view returns (uint64 count, int128 summaryValue, uint8 summaryValueDecimals) {
        require(participantRegistry_ != address(0), "participantRegistry required");

        ReputationRegistryStorage storage $ = _getReputationRegistryStorage();
        address[] memory allClients = $._clients[agentId];
        ...
        for (uint256 i; i < allClients.length; i++) {
            address client = allClients[i];

            // P3 gate: only count feedback from trusted institutions.
            bytes32 clientPid = IParticipantRegistryExt(participantRegistry_)
                .getMinterParticipantId(client);
            if (!_isInList(clientPid, trustedParticipantIds)) continue;
        ...
        uint64 maxCount_;
        for (uint8 d; d <= 18; d++) {
            if (decimalCounts[d] > maxCount_) { maxCount_ = decimalCounts[d]; modeDecimals = d; }
        }
        int256 avgWad = sum / int256(uint256(count));
        summaryValue         = int128(avgWad / int256(10 ** uint256(18 - modeDecimals)));
        summaryValueDecimals = modeDecimals;
    ...
    function _isInList(bytes32 value, bytes32[] calldata list) internal pure returns (bool) {

...
**Deploy wiring.** `scripts/deploy.js` reads the raw card bytes, computes the hash, and
...
| Risk Management | Flow anomaly detection    | `ExecutionTraceLog` (on-chain) + `bounds_monitor.py` (off-chain) | Does this flow exhibit **structural anomalies** — loops, runaway depth, bursts, or stuck responses? |

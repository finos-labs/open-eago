"""
hf_legal_bridge.py — Port of hf-legal-bridge.js using web3.py AsyncWeb3.

Watches LegalOracle.DraftIssued events and submits markup via the HF legal MCP server.
Human approval (approveClientSide) is a separate external action.

Usage:
    python -m bridges.hf_legal_bridge \
        --legal-contract 0x... --rpc http://... --privkey 0x... --agent-id 9
ENV: LEGAL_CONTRACT_ADDRESS, RPC_URL, ORACLE_PRIVATE_KEY, HF_LEGAL_AGENT_ID
"""

from __future__ import annotations

import asyncio
import logging
import sys

from shared.abis import LEGAL_ORACLE_ABI
from shared.bridge_base import arg, build_web3, call_mcp_tool
from shared.vault_signer import create_signer

logger = logging.getLogger(__name__)

LABEL        = "hf-legal-bridge"
MCP_ENDPOINT = arg("--mcp-endpoint", "HF_LEGAL_MCP_ENDPOINT", "http://localhost:8022")

RPC_URL        = arg("--rpc",          "RPC_URL",             "http://127.0.0.1:8545")
PRIVATE_KEY    = arg("--privkey",      "ORACLE_PRIVATE_KEY")
SIGNER_TYPE    = arg("--signer-type",  "SIGNER_TYPE",         "local")
VAULT_URL      = arg("--vault-url",    "VAULT_URL")
VAULT_ADDRESS  = arg("--vault-address","ORACLE_ADDRESS")
AGENT_ID       = int(arg("--agent-id", "HF_LEGAL_AGENT_ID",  "9"))

LEGAL_CONTRACT = arg("--legal-contract", "LEGAL_CONTRACT_ADDRESS")


def _hex(b) -> str:
    return b.hex() if isinstance(b, bytes) else str(b).removeprefix("0x")


def _bytes(h: str) -> bytes:
    return bytes.fromhex(h.removeprefix("0x"))


async def _handle_draft_issued(oracle, event) -> None:
    args          = event["args"]
    request_id    = args["requestId"]
    flow_id       = args["flowId"]
    contract_hash = args["contractHash"]
    round_num     = args["round"]
    flow_hex      = _hex(flow_id)

    logger.info("\n[%s] ← DraftIssued  requestId=%s  round=%d", LABEL, _hex(request_id), round_num)

    try:
        result = await call_mcp_tool(
            MCP_ENDPOINT, "review_draft",
            {
                "flow_id":    flow_hex,
                "request_id": _hex(request_id),
                "draft_hash": _hex(contract_hash),
                "round":      int(round_num),
            },
            flow_hex,
        )

        logger.info(
            "[%s]   → submitMarkup  changes=%s  hash=%s",
            LABEL, result.get("changes"), result["markup_hash"],
        )
        req_bytes = request_id if isinstance(request_id, bytes) else _bytes(_hex(request_id))
        tx = await oracle.functions.submitMarkup(
            req_bytes, AGENT_ID, _bytes(result["markup_hash"])
        ).transact()
        logger.info("[%s]   ✓ submitMarkup  tx=%s", LABEL, tx.hex())
    except Exception as exc:
        logger.error("[%s]   ✗ %s", LABEL, exc)


def _handle_in_human_review(event) -> None:
    args = event["args"]
    logger.info(
        "\n[%s] *** InHumanReview  requestId=%s  round=%s",
        LABEL, _hex(args["requestId"]), args["round"],
    )
    logger.info("[%s] *** HF human approver: call approveClientSide(requestId, %d)", LABEL, AGENT_ID)


async def main() -> None:
    if not LEGAL_CONTRACT:
        logger.error("[%s] Missing --legal-contract", LABEL); sys.exit(1)
    if SIGNER_TYPE == "local" and not PRIVATE_KEY:
        logger.error("[%s] Missing --privkey",        LABEL); sys.exit(1)

    signer = create_signer(SIGNER_TYPE, PRIVATE_KEY, VAULT_URL, VAULT_ADDRESS)
    w3, _  = build_web3(RPC_URL, signer)
    oracle = w3.eth.contract(address=LEGAL_CONTRACT, abi=LEGAL_ORACLE_ABI)

    logger.info("[%s] Signer        : %s", LABEL, signer.address)
    logger.info("[%s] LegalOracle   : %s", LABEL, LEGAL_CONTRACT)
    logger.info("[%s] AgentId       : %d", LABEL, AGENT_ID)
    logger.info("[%s] MCP endpoint  : %s", LABEL, MCP_ENDPOINT)
    logger.info("[%s] Listening for DraftIssued / InHumanReview events…", LABEL)

    draft_filter  = await oracle.events.DraftIssued.create_filter(from_block="latest")
    human_filter  = await oracle.events.InHumanReview.create_filter(from_block="latest")

    while True:
        for event in await draft_filter.get_new_entries():
            asyncio.create_task(_handle_draft_issued(oracle, event))
        for event in await human_filter.get_new_entries():
            _handle_in_human_review(event)
        await asyncio.sleep(2)


if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO)
    asyncio.run(main())

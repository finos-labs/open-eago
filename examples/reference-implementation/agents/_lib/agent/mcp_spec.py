...
def build_tools_from_spec(mcp_spec: dict, config: dict, extra_handlers: dict | None = None) -> dict:
    """Build the tool dispatch table.

    `extra_handlers` lets a phase agent (examples/reference-implementation) plug in
    real business logic for tools beyond the built-in `eago_health`, keyed by tool name.
    """
